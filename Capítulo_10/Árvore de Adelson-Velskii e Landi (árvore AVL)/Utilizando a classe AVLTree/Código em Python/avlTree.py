...
class AVLTree(BinarySearchTree):
    ...
    # Método get_node_height
    def get_node_height(self, node):
        if node is None:
            return -1
        return max(self.get_node_height(node.left), self.get_node_height(node.right)) + 1
    
    # Método get_balance_factor
    def get_balance_factor(self, node):
        height_difference = self.get_node_height(node.left) - self.get_node_height(node.right)
        return {
            -2: balance_factor["UNBALANCED_RIGHT"],
            -1: balance_factor["SLIGHTLY_UNBALANCED_RIGHT"],
            1: balance_factor["SLIGHTLY_UNBALANCED_LEFT"],
            2: balance_factor["UNBALANCED_LEFT"]
        }.get(height_difference, balance_factor["BALANCED"])
    ...
    # Método rotation_rl
    # Caso direita esquerda: gire para a direita e depois para a esquerda
    def rotation_rl(self, node):
        node.right = self.rotation_ll(node.right)
        return self.rotation_rr(node)
    
    # Método rotation_lr
    # Caso esquerda direita: gire para a esquerda e depois para a direita
    ...
    # Método insert
    def insert(self, key):
        self.set_root(self.insert_node(self.get_root(), key))
    ...
    def insert_node(self, node, key):
        ...
        if node is None:
            return Node(key)
        elif self.compare_fn(key, node.key) == Compare["LESS_THAN"]:
        ...
        if balance_fac == balance_factor["UNBALANCED_LEFT"]:
            if self.compare_fn(key, node.key) == Compare["LESS_THAN"]:
                node = self.rotation_ll(node)
            else:
                return self.rotation_lr(node)
        
        if balance_fac == balance_factor["UNBALANCED_RIGHT"]:
            if self.compare_fn(key, node.key) == Compare["BIGGER_THAN"]:
    ...
    def remove_node(self, node, key):
        ...
        if node is None:
            return node # None, não é necessário balancear
        
        # verifica se a árvore está balanceada
        balance_fac = self.get_balance_factor(node)
        if balance_fac == balance_factor["UNBALANCED_LEFT"]:
            balance_fac_left = self.get_balance_factor(node.left)
            if balance_fac_left == balance_factor["BALANCED"] or balance_fac_left == balance_factor["SLIGHTLY_UNBALANCED_LEFT"]:
                return self.rotation_ll(node)
            if balance_fac_left == balance_factor["SLIGHTLY_UNBALANCED_RIGHT"]:
                return self.rotation_lr(node.left)
        if balance_fac == balance_factor["UNBALANCED_RIGHT"]:
            balance_fac_right = self.get_balance_factor(node.right)
            if balance_fac_right == balance_factor["BALANCED"] or balance_fac_right == balance_factor["SLIGHTLY_UNBALANCED_RIGHT"]:
                return self.rotation_rr(node)
            if balance_fac_right == balance_factor["SLIGHTLY_UNBALANCED_LEFT"]:
                return self.rotation_rl(node.right)
        return node  
...
avlTree.in_order_traverse(print_node) # Imprime os valores em ordem crescente
